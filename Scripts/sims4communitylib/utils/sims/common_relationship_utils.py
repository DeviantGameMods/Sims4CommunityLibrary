...
class CommonRelationshipUtils(_HasS4CLClassLog):
    ...
    def get_log_identifier(cls) -> str:
        return 'common_relationship_utils'
    ...
    def are_blood_relatives(cls, sim_info_a: SimInfo, sim_info_b: SimInfo) -> bool:
        ...
        return CommonSimFamilyTreeUtils.are_blood_related(sim_info_a, sim_info_b)
...
@CommonConsoleCommand(
    ModInfo.get_identity(),
    's4clib.run_test_based_score_on_sim',
    'Run a Test Based Score with one Sim',
    command_arguments=(
        CommonConsoleCommandArgument('sim_info', 'Name or Id', 'The name of a Sim. The test based score will be run with this Sim.'),
        CommonConsoleCommandArgument('test_based_score', 'Name or Id', 'The name or id of the Test Based Score instance.')
    ),
    command_aliases=(
        's4clib.runtestbasedscoreonsim',
    )
)
def _common_run_test_based_score_on_sim(
    output: CommonConsoleCommandOutput,
    sim_info: SimInfo,
    test_based_score: TunableInstanceParam(Types.TEST_BASED_SCORE)
):
    if sim_info is None:
        output('ERROR: No Target was specified!')
        return False
    resolver = SingleSimResolver(sim_info)
    result_score = test_based_score.get_score(resolver)
    output(f'Ran test based score on {sim_info}, got score {result_score}')

    passing_score_breakdown = list()
    failing_score_breakdown = list()
    scores = None
    for test_pair in test_based_score._scores:
        if resolver(test_pair.test):
            score = test_pair.score
            if scores is None:
                scores = score
            else:
                scores += score
            passing_score_breakdown.append(f'Score Test: {test_pair}, Score: {score}')
        else:
            failing_score_breakdown.append(f'Score Test: {test_pair}')

    batch_test_scores = 0
    for batch_score_test in test_based_score._batch_test_scores:
        result = resolver(batch_score_test.test)
        if result:
            if isinstance(result, TestResultNumeric):
                score = result.current_value
                batch_test_scores += score
                passing_score_breakdown.append(f'Batch Score Test: {batch_score_test}, Score: {score}')
        else:
            failing_score_breakdown.append(f'Batch Score Test: {batch_score_test}')
    log.format_with_message('Passing Scores:')
    for passing_score_breakdown_entry in passing_score_breakdown:
        log.format_with_message(passing_score_breakdown_entry)
    log.format_with_message('Failing Scores:')
    for failing_score_breakdown_entry in failing_score_breakdown:
        log.format_with_message(failing_score_breakdown_entry)


# noinspection SpellCheckingInspection
@CommonConsoleCommand(
    ModInfo.get_identity(),
    's4clib.run_test_based_score_on_sims',
    'Run a Test Based Score with two Sims',
    command_arguments=(
        CommonConsoleCommandArgument('sim_a', 'Name or Id', 'The name of the first Sim. The test based score will be tested from Sim A to Sim B.'),
        CommonConsoleCommandArgument('sim_b', 'Name or Id', 'The name of the second Sim. The test based score will be tested from Sim A to Sim B.'),
        CommonConsoleCommandArgument('test_based_score', 'Name or Id', 'The name or id of the Test Based Score instance.')
    ),
    command_aliases=(
        's4clib.runtestbasedscoreonsims',
    )
)
def _common_run_test_based_score_on_sims(
    output: CommonConsoleCommandOutput,
    sim_info_a: SimInfo,
    sim_info_b: SimInfo,
    test_based_score: TunableInstanceParam(Types.TEST_BASED_SCORE)
    ...
    if sim_info_b is None:
        ...
        return False
    if sim_info_a is sim_info_b:
        output('ERROR: Cannot apply to the same Sim.')
        return False
    resolver = DoubleSimResolver(sim_info_a, sim_info_b)
    result_score = test_based_score.get_score(resolver)
    output(f'Ran test based score from {sim_info_a} to {sim_info_b}, got score {result_score}')
