...
class CommonLocationUtils:
    ...
    @staticmethod
    def get_zone(zone: Union[int, Zone], allow_unloaded_zones: bool = False) -> Zone:
        """get_zone(zone, allow_unloaded_zones=False)

        Retrieve a Zone instance.

        :param zone: The identifier of a Zone.
        :type zone: Union[int, Zone]
        ...
        if isinstance(zone, Zone):
            return zone
        return services.get_zone(zone, allow_uninstantiated_zones=allow_unloaded_zones)
    ...
    @classmethod
    def get_venue_service_for_current_zone(cls) -> VenueService:
        """get_venue_service_for_current_zone()

        Get the service for Venues for the current Zone.

        :return: The venue service for the current zone.
        :rtype: VenueService
        """
        # noinspection PyUnresolvedReferences
        return cls.get_current_zone().venue_service

    @classmethod
    def get_venue_service(cls, zone: Union[int, Zone]) -> Union[VenueService, None]:
        """get_venue_service(zone)

        Get the service for Venues for a Zone.

        :param zone: The zone to retrieve the service from.
        :type zone: Union[int, Zone]
        :return: The venue service for the specified zone or None if no service found.
        :rtype: Union[VenueService, None]
        """
        zone_instance = cls.get_zone(zone)
        if zone_instance is None:
            return None
        # noinspection PyUnresolvedReferences
        return zone_instance.venue_service
